...
def totalLoss(output, targets):
    loss1 = GeneralizedDiceLoss(output, targets[:, 0, ::])
    loss2 = GeneralizedDiceLoss(output, targets[:, 1, ::])
    loss3 = GeneralizedDiceLoss(output, targets[:, 2, ::])
    return  loss1 + loss2 + loss3