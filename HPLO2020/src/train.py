...
from src.loss import criterions
# Unet  HPLONet
parser = argparse.ArgumentParser(description='Brats Training')
parser.add_argument('-a', '--arch', metavar='ARCH', default='DMFNet',
                    help='model architecture (default: HPLONet)')
...
parser.add_argument('--full', default=True, action='store_true', help='Fit the network on the full training set')
...
def main(args):
    ...
    # model = model_maker(c=4,n=32,channels=128,groups = 16,norm='sync_bn', num_classes=3)
    print(f"total number of trainable parameters {count_parameters(model)}")

    # if args.swa:
    #     # Create the average model
    #     print('Create the average model')
    ...
    criterion = EDiceLoss(False).cuda()
...
if __name__ == '__main__':
    ...
    #     width=args.width, deep_supervision=args.deep_sup,
    #     norm_layer=get_norm_layer(args.norm_layer), dropout=args.dropout)
    # input = torch.rand((1, 4, 128, 128, 128))  # [bsize,channels,Height,Width,Depth]
    # ckpt = "/media/gxx/Data2/lht/program-py/HPLO2020/runs/20220624_003207__foldFULL_HPLONet_48_batch1_optimadam_adam_lr0.0001-wd0.0_epochs5_deepsupFalse_fp16_warm3__normgroup_dropout0.0_warm_restartFalse/model_best.pth.tar"
    # checkpoint = torch.load(ckpt)
    # start_epoch = checkpoint['epoch']
    #
    # model.load_state_dict(checkpoint['state_dict'])

    # print(model['module.ftr.pre_head_ln.bias'])
    # # target = torch.rand((1, 4, 128, 128, 128), device=device)
    # # #
    # # model.cuda(device)
    #
    # flops, params = profile(model, inputs=(input,))
    # macs, params = clever_format([flops, params], "%.3f")
    # print('Number of FLOPs:', macs)
